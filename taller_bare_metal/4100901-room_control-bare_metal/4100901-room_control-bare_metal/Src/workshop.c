...
// --- Definiciones para el botón en PC13 ----------------------------
#define GPIOC_BASE  0x48000800U
#define GPIOC_MODER (*(volatile uint32_t *)(GPIOC_BASE + 0x00U)) // Modo
#define GPIOC_IDR   (*(volatile uint32_t *)(GPIOC_BASE + 0x10U)) // Entrada
#define BTN_PIN     13U                                          // Pin PC13
...
int main(void)
{
    ...
    while (1) {
        // Botón presionado = nivel bajo (bit 13 == 0)
        if ((GPIOC_IDR & (1 << BTN_PIN)) == 0) {
            GPIOA_ODR |=  (1 << LD2_PIN); // Enciende PA5
            delay_3s();                   // Espera ~3 s
            GPIOA_ODR &= ~(1 << LD2_PIN); // Apaga PA5
        }
    }
}
...
void init_button(void)
{
    RCC_AHB2ENR |= (1 << 2);                       // Habilita reloj para GPIOC
    GPIOC_MODER &= ~(3 << (BTN_PIN*2));            // PC13 como entrada (00)
...
void delay_3s(void)
{
    // Aproximado: 3 segundos con reloj interno de 4 MHz
    // Cada NOP ~1 ciclo
    for (volatile uint32_t i = 0; i < (HSI_FREQ * 3); i++) {
        __asm volatile("nop");
    }
}
