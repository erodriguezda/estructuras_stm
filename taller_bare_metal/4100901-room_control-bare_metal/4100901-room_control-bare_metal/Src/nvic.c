...
{
    NVIC->IP[IRQn] = (priority << 4);  // Solo los 4 bits altos son válidos
}
...
void nvic_exti_pc13_button_enable(void) {
    ...
    nvic_set_priority(EXTI15_10_IRQn, 1); // Prioridad más alta (botón)
...
void nvic_usart2_irq_enable(void) {
    ...
    nvic_set_priority(USART2_IRQn, 2); // Prioridad más baja (UART)
